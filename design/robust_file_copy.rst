...
4. [.] **autoname**: construct the filename guess
...
5. **prompt**: Prompt for name
   ...
   **Skip this task if ``.filename_provided`` exists,
   ...
   a. Suggest TransferInfo.fname_guess

   b. Prompt for the final filename
   ...
   d. Dump the response filename to ``.filename_provided``::

       echo $filename_provided > .filename_provided

6. **flacenc**: Flac encode
...
7. **pargen**: Rename and par2 dest flac file
...
9. **cleanup**: Delete src wav and copy back flac
...
10. **finish**: *[global]* Wait for all processing to complete
